...
def compress():
    ...
    # Symbol frequencies (byte value → count)
    symbol_frequencies = {}
    for byte in raw:
        symbol_frequencies[byte] = symbol_frequencies.get(byte, 0) + 1

    # Serialize final Huffman tree
    def serialize_tree(node):
        if node is None:
            return None
        if node.is_leaf() and not node.is_nyt:
            return {
                "type":   "leaf",
                "char":   chr(node.symbol) if 32 <= node.symbol <= 126 else f"\\x{node.symbol:02x}",
                "weight": node.weight,
                "code":   encoder._tree.get_code(node),
            }
        return {
            "type":   "internal",
            "weight": node.weight,
    ...
    huffman_tree = serialize_tree(encoder._tree.root)
